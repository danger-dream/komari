...
function SelectorInner<T>(props: SelectorProps<T>) {
	...
	const [viewMode, setViewMode] = React.useState<'list' | 'group'>(defaultViewMode ?? 'list')
	...
	const enableGroupView = !!groupBy && (!viewModeSwitch || viewMode === 'group')
	...
	const groupedItems = React.useMemo(() => {
		...
		return entries
	}, [processed, groupBy, enableGroupView])
				...
				{viewModeSwitch && groupBy && (
					<div className="flex gap-1 border border-(--gray-6) rounded-md p-0.5">
						<button
							...
							title={String(viewModeLabels?.list ?? '列表视图')}>
							<List size={14} />
						</button>
						<button
							...
							title={String(viewModeLabels?.group ?? '分组视图')}>
							<LayoutGrid size={14} />
						</button>
					</div>
				)}
											...
											{(group || ungroupedLabel) && (
												<Flex align="center" gap="2" className="px-2 py-1 mb-1">
													...
													<Text size="2" weight="bold" className="text-(--gray-11)">
														{group || ungroupedLabel} ({items.length})
													</Text>
												</Flex>
											)}
											{/* 分组项目 */}
