...
import { Button, Checkbox, Dialog, Flex, IconButton, SegmentedControl, TextArea, TextField } from '@radix-ui/themes'
...
export function ActionsCell({ row }: { row: Row<z.infer<typeof schema>> }) {
	...
	const generateCommand = () => {
		...
		switch (selectedPlatform) {
			case 'linux':
				finalCommand = `bash <(curl -sL https://raw.githubusercontent.com/komari-monitor/komari-agent/refs/heads/main/install.sh) ` + args.join(' ')
				break
			case 'windows':
				finalCommand =
				...
				break
			case 'macos':
				finalCommand = `zsh <(curl -sL https://raw.githubusercontent.com/komari-monitor/komari-agent/refs/heads/main/install.sh) ` + args.join(' ')
				break
		}
		return finalCommand
	}
				...
				<Dialog.Content>
					...
					<div className="flex flex-col gap-4">
						...
						<Flex direction="column" gap="2">
							...
							<Flex direction="column" gap="2">
								<label className="text-sm font-bold">{t('admin.nodeTable.ghproxy', 'GitHub 代理')}</label>
								<TextField.Root
									placeholder={t('admin.nodeTable.ghproxy_placeholder', 'GitHub 代理，为空则不使用代理')}
									onChange={e =>
										setInstallOptions(prev => ({
											...prev,
								...
								<label className="text-sm font-bold">{t('admin.nodeTable.install_dir', '安装目录')}</label>
								<TextField.Root
								...
								<label className="text-sm font-bold">{t('admin.nodeTable.serviceName', '服务名称')}</label>
								<TextField.Root
