	...
	return (
		<form onSubmit={handleCreate} className="flex flex-col gap-4">
			...
			<label className="flex flex-col gap-2">
				...
				<TextArea placeholder={t('agent_version.changelog_placeholder')} value={formChangelog} onChange={e => setFormChangelog(e.target.value)} rows={5} />
			</label>
		...
		<>
					...
					<Flex gap="1" wrap="wrap">
						{version.packages && version.packages.length > 0 ? (
							version.packages.map(p => (
								<PackageBadge key={p.id} pkg={p} versionId={version.id} onUpdate={onUpdate} />
							))
						) : (
							<Text size="1" color="gray">
					...
					<Flex gap="2" justify="center">
						{!version.is_current && (
							<IconButton
								...
								title={t('agent_version.mark_current')}>
								<Check size={14} />
							</IconButton>
						)}
			...
			<Dialog.Root open={deleteOpen} onOpenChange={setDeleteOpen}>
				<Dialog.Content style={{ maxWidth: 450 }}>
					...
					<Text size="2">
						{t('agent_version.delete_confirm', { version: version.version })}
					</Text>
		...
		<>
			<Dialog.Root open={deleteOpen} onOpenChange={setDeleteOpen}>
				<Flex
					align="center"
					gap="2"
					className="px-2 py-1 rounded-md group relative"
					...
					<Flex gap="2" className="ml-2">
						<IconButton
							...
							title={t('agent_version.download')}>
							<Download size={12} />
						</IconButton>
						<Dialog.Trigger>
							<IconButton
