...
import { Copy, Download, MenuIcon, Pencil, Plus, Terminal, Trash2Icon } from 'lucide-react'
...
const Layout = () => {
	...
	return (
		<Flex direction="column" gap="4" p="4">
			<Header searchTerm={searchTerm} setSearchTerm={setSearchTerm} selectedNodes={selectedNodes} viewMode={viewMode} setViewMode={setViewMode} />

			<NodeTable nodes={filteredNodes} selectedNodes={selectedNodes} setSelectedNodes={setSelectedNodes} viewMode={viewMode} />
		</Flex>
	)
}
...
const Header = ({
	...
	return (
		<Flex justify="between" align="center" gap="4" wrap="wrap">
			...
			<Flex gap="2" align="center">
				<SegmentedControl.Root size="2" value={viewMode} onValueChange={v => setViewMode(v as 'list' | 'group')}>
					<SegmentedControl.Item value="list">{t('admin.nodeTable.view.list', '列表')}</SegmentedControl.Item>
					<SegmentedControl.Item value="group">{t('admin.nodeTable.view.group', '分组')}</SegmentedControl.Item>
				...
				<Dialog.Root open={dialogOpen} onOpenChange={setDialogOpen}>
					...
					<Dialog.Content>
						...
						<TextField.Root ref={inputRef} placeholder={t('admin.nodeTable.nameOptional')} />
						<Flex justify="end" gap="2" mt="4">
							<Button onClick={() => handleAddNode(inputRef.current?.value)} disabled={loading}>
								{t('admin.nodeTable.addNode')}
							</Button>
						</Flex>
					</Dialog.Content>
...
const SortableRow = ({
	...
	return (
		<TableRow ref={setNodeRef} style={style} className="hover:bg-accent-a2">
			...
			<TableCell>
				<Checkbox checked={selectedNodes.includes(node.uuid)} onCheckedChange={checked => handleSelectNode(node.uuid, !!checked)} />
			</TableCell>
			<TableCell>
			...
			<TableCell>
				<Flex direction="column">
					...
					{node.ipv6 && (
						<Text size="2" className="flex items-center gap-1" title={node.ipv6}>
							{node.ipv6.length > 20
								? (() => {
										const segments = node.ipv6.split(':')
										return segments.length > 3 ? `${segments.slice(0, 2).join(':')}:...${segments[segments.length - 1]}` : node.ipv6
								  })()
								: node.ipv6}
			...
			<TableCell>
				<Text
					...
					{node.remark && node.remark.length > 10 ? `${node.remark.slice(0, 10)}...` : node.remark}
				</Text>
			</TableCell>
			<TableCell>
...
const NodeTable = ({
	...
	return (
		<div className={`rounded-md overflow-hidden ${isDragging ? 'select-none' : ''}`}>
			<DndContext
				sensors={sensors}
				...
				<Table>
					<TableHeader style={{ backgroundColor: 'var(--accent-4)' }}>
						<TableRow>
							<TableHead></TableHead>
							<TableHead>
								<Checkbox checked={selectedNodes.length === localNodes.length && localNodes.length > 0} onCheckedChange={handleSelectAll} />
							</TableHead>
					...
					<TableBody>
						<SortableContext items={localNodes.map(node => node.uuid)} strategy={verticalListSortingStrategy} disabled={viewMode === 'group'}>
							{viewMode === 'group'
								? groupedNodes.flatMap(([groupName, nodes]) => {
										const items: React.ReactNode[] = []
										items.push(
											<TableRow key={`group-${groupName || 'ungrouped'}`} className="bg-[var(--accent-3)]">
												<TableCell colSpan={8} className="text-xs font-medium text-[var(--accent-11)] uppercase tracking-wide">
													{groupName || t('common.ungrouped', { defaultValue: '未分组' })}
												</TableCell>
											</TableRow>
										)
										nodes.forEach(node => {
...
function EditButton({ node }: { node: NodeDetail }) {
	...
	const [tab, setTab] = useState<'basic' | 'billing'>('basic')
	...
	const initialForm = React.useMemo(
		() => ({
			...
			billingMode: billingPresets.find(b => b.value === node.billing_cycle.toString())?.value || 'custom',
			customBilling: billingPresets.find(b => b.value === node.billing_cycle.toString()) ? '' : node.billing_cycle.toString(),
			...
			<Dialog.Content>
				...
				<Tabs.Root value={tab} onValueChange={v => setTab(v as any)}>
					<Tabs.List>
						...
						<Tabs.Trigger value="billing">{t('admin.nodeTable.billing', '账单')}</Tabs.Trigger>
					</Tabs.List>
					<Tabs.Content value="basic">
						<div className="flex flex-col gap-4 pt-3">
							...
							<div>
								...
								<TextArea
									value={form.publicRemark}
									onChange={e => setForm(f => ({ ...f, publicRemark: e.target.value }))}
									resize={'vertical'}
								/>
							</div>
							<div>
							...
							<div className="rounded-md border border-[var(--accent-5)] bg-[var(--accent-2)] p-3">
