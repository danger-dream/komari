...
import { Plus, RefreshCw, Save, Settings, Pencil, Trash2, Terminal, Shield, Code, Copy, CheckCircle2, Calendar, Hash, Globe2, KeyRound, List, LayoutGrid } from 'lucide-react'
...
const NodeSelectorBox: React.FC<NodeSelectorBoxProps> = ({ nodes, selectedNodes, onChange }) => {
	const [viewMode, setViewMode] = React.useState<'list' | 'group'>('list')
	const [searchText, setSearchText] = React.useState('')

	// 按权重排序节点
	const sortedNodes = React.useMemo(() => {
		return [...nodes].sort((a, b) => (a.weight ?? 0) - (b.weight ?? 0))
	}, [nodes])
	...
	const filteredNodes = React.useMemo(() => {
		...
		return sortedNodes.filter(n => n.name.toLowerCase().includes(lower) || n.uuid.toLowerCase().includes(lower))
	...
	const toggleNode = (uuid: string) => {
		const exists = selectedNodes.includes(uuid)
		onChange(exists ? selectedNodes.filter(id => id !== uuid) : [...selectedNodes, uuid])
	}
	...
	const toggleAll = () => {
		if (selectedNodes.length === filteredNodes.length) {
			...
			onChange(filteredNodes.map(n => n.uuid))
		}
	}
		...
		<div className="space-y-3 flex flex-col">
			<div className="flex items-center justify-between">
				...
				<Flex gap="2" align="center">
					...
					<div className="flex gap-1 border border-(--gray-6) rounded-md p-0.5">
						<button
							onClick={() => setViewMode('list')}
							className={`p-1 rounded transition-colors ${
								viewMode === 'list' ? 'bg-accent-3 text-accent-11' : 'text-(--gray-10) hover:bg-(--gray-3)'
							}`}
							title="列表视图">
							<List size={14} />
						</button>
						<button
							...
							className={`p-1 rounded transition-colors ${
								viewMode === 'group' ? 'bg-accent-3 text-accent-11' : 'text-(--gray-10) hover:bg-(--gray-3)'
							}`}
							title="分组视图">
							<LayoutGrid size={14} />
						</button>
			...
			{/* 搜索框 */}
			<TextField.Root placeholder="搜索节点..." value={searchText} onChange={e => setSearchText(e.target.value)} size="2" />
							...
							<div className="space-y-0">
								{filteredNodes.map(n => (
									<label
										...
										<Checkbox checked={selectedNodes.includes(n.uuid)} onCheckedChange={() => toggleNode(n.uuid)} />
										<Flag flag={n.region ?? ''} size="4" />
										<span className="text-sm truncate leading-tight flex-1">{n.name}</span>
										{n.group && <Badge size="1" variant="surface" color="gray">{n.group}</Badge>}
									</label>
								))}
								{filteredNodes.length === 0 && <div className="text-sm text-(--gray-9) text-center py-4">暂无节点</div>}
							</div>
						) : (
							<div className="space-y-3">
								{Object.entries(groupedNodes).map(([groupName, groupNodes]) => (
									<div key={groupName}>
										<Text size="2" weight="bold" className="px-2 py-1 text-(--gray-11)">
										...
										<div className="space-y-0 mt-1">
											{groupNodes.map(n => (
												<label
													...
													<Checkbox checked={selectedNodes.includes(n.uuid)} onCheckedChange={() => toggleNode(n.uuid)} />
													<Flag flag={n.region ?? ''} size="4" />
													<span className="text-sm truncate leading-tight">{n.name}</span>
												</label>
											))}
										</div>
									</div>
								))}
								{Object.keys(groupedNodes).length === 0 && <div className="text-sm text-(--gray-9) text-center py-4">暂无节点</div>}
							</div>
						)}
					</div>
				</ScrollArea>
			...
			<Text size="1" color="gray">
				已选择 {selectedNodes.length} / {filteredNodes.length} 个节点
			</Text>
		</div>
	)
}
...
const ToolSelectorBox: React.FC<ToolSelectorBoxProps> = ({ selectedTools, onChange }) => {
	...
	return (
		<div className="space-y-3 flex flex-col">
			...
			<Text size="1" color="gray">
				已选择 {selectedTools.length} / {filteredTools.length} 个工具
			</Text>
		</div>
	)
}
