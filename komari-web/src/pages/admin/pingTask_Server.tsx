...
import { ClipboardCopy, ClipboardPaste, MoreHorizontal, List, LayoutGrid } from 'lucide-react'
...
export const ServerView = ({ pingTasks }: { pingTasks: PingTask[] }) => {
	...
	const sortedNodes = React.useMemo(() => {
		const nodes = [...nodeDetail].sort((a, b) => {
			const wa = a.weight ?? 0
			const wb = b.weight ?? 0
	...
	const groupedNodes = React.useMemo(() => {
		const groups = new Map<string, (typeof sortedNodes)>()
			...
			<Flex justify="end" className="mb-3">
				<div className="flex gap-1 border border-(--gray-6) rounded-md p-0.5">
					<button
						...
						title={t('admin.nodeTable.view.list', '列表视图')}>
						<List size={14} />
					</button>
					<button
						...
						title={t('admin.nodeTable.view.group', '分组视图')}>
						<LayoutGrid size={14} />
					</button>
				</div>
			</Flex>
							...
							? groupedNodes.map(([groupName, nodes]) => (
									<React.Fragment key={groupName || 'ungrouped'}>
										<TableRow className="bg-accent-3">
											<TableCell colSpan={3} className="text-xs font-medium text-accent-11 uppercase tracking-wide">
												{groupName || t('common.ungrouped', { defaultValue: '未分组' })} ({nodes.length})
											</TableCell>
										</TableRow>
										{nodes.map(n => (
											<ServerRow
												...
												onCopyBufferChange={setCopyBuffer}
											/>
										))}
									</React.Fragment>
							  ))
